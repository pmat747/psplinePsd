...
gibbs_pspline_simple <- function(data,
  ...
  knots = knotLoc(data = data, k = k, degree = degree, eqSpaced = eqSpacedKnots,
                  nfreqbin = nfreqbin, wfreqbin = wfreqbin);
  ...
  if(eqSpacedKnots == TRUE){
    ...
    P       = diffMatrix(k, d = diffMatrixOrder); # Third order penalty
    P       = t(P) %*% P;
  ...
  sigtaStore = NULL
  sigtalpha = 100;    # variance of proposal distb for tau
  ...
  Zt = stats::rnorm((Ntotal-1)*k1); # random values for tau
  Zt = matrix(Zt, nrow = Ntotal-1, ncol = k1);
  Ut = log(stats::runif((Ntotal-1)*k1, 0, 1));
  Ut = matrix(Ut, nrow = Ntotal-1, ncol = k1);
  ...
  for(i in 1:length(keep)){
    ...
    ll.trace = c(ll.trace,
                 llike(omega, FZ, k, V[,i], tau[i], tau.alpha, pdgrm, degree, db.list,
                       spec_ar));
  }
  ...
  for (isample in 1:length(keep)) {
    ...
    fpsd.sample[, isample] <- tau[isample] + q.psd;  # CHANGE FOR LOG TRANSFORMATION
  ...
  l = llike(omega, FZ, k, v = v_means,
            tau = tau_mean,tau.alpha, pdgrm, degree, db.list, spec_ar);
  ...
  anSpecif = list(k = k, knots = knots, n = n, degree = degree, FZ = FZ,
  ...
  output = list(psd.median = psd.median * rescale ^ 2,
                ...
                fpsd.sample = fpsd.sample + 2*log(rescale),#fpsd.sample * rescale ^ 2,
                anSpecif = anSpecif,
                n = n,
                tau = tau,
                phi = phi,
